...
OUTPUT_FILE = open('/media/ivan/DDE/datasets_proyecto_mt/notop/en_test.csv', "w")
...
def get_list_of_labels(image_path, include_top=True):
    ...
    if not include_top:
        return np.reshape(features, 7 * 7 * 512)
    return features
...
def label_author_photos(author_dir_path):
    ...
    author_id = author_dir_path
    for image_file in list_of_files:
        labels = get_list_of_labels(BASE_DIR_PHOTOS + '/' + author_dir_path + '/' + image_file, False)
        if labels is None:
            continue
        if not all_vectors.any():
            all_vectors = labels
        else:
            all_vectors = np.add(all_vectors, labels)
        counter += 1
    line = ', '.join(map(str, list(all_vectors / counter)))
    line = author_id + ', ' + line
    OUTPUT_FILE.write(line + ', ' + GENDER_LABELS[author_id] + '\n')
        # if len(labels) == 7 * 7 * 512:
        #     line = ', '.join(map(str, labels))
        # else:
        #     line = ', '.join(map(str, labels[0]))
        # IDX += 1
        # line = author_id + ', ' + line
        # OUTPUT_FILE.write(line + ', ' + GENDER_LABELS[author_id] + '\n')
